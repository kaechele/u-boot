...
#define PORT_SWITCH_ID_6393X		0x3930
...
static int mv88e6xxx_port_probe(struct udevice *dev, int port, struct phy_device *phy)
{
	...
	if (mv88e6xxx_amethyst_family(dev)) {
		...
		case 10:
			...
			if (ret) {
				dev_err(dev, "failed to power port %d SERDES: %d\n", port, ret);
				return ret;
			}
			break;
		default:
...
static int mv88e6xxx_probe(struct udevice *dev)
{
	...
	if (mv88e6xxx_amethyst_family(dev)) {
		...
		case PHY_INTERFACE_MODE_10GBASER:
			cmode = PORT_REG_STATUS_CMODE_10GBASE_R;
			break;
		default:
		...
		ret = mv88e6xxx_set_cmode(dev, 10, PORT_REG_STATUS_CMODE_10GBASE_R);
		if (ret < 0)
			return ret;
	}
